...
public class PageInfo {
	...
	private int recordCountPerPage = 10; //한 페이지당 게시되는 게시물 건 수	yes	
	...
	public void makePageHtml() {
		...
		pageHtml += "<a href=\"#\" onclick=\"goPage(" + ( currentPageNo == 1? 1 : (currentPageNo-1) ) + "); return false;\">[이전]</a>";
		for (int i = firstPageNoOnPageList; i <= lastPageNoOnPageList; i++) {
			if (i == currentPageNo) {
				pageHtml += "<strong>【" + i + "】</strong>"; //현재페이지
			}else {
				pageHtml += "<a href=\"#\" onclick=\"goPage(" + i + "); return false;\">【" + i + "】</a>";
			}
		}
		//pageHtml += "<a href=\"#\" onclick=\"goPage(" + (currentPageNo + 1) + "); return false;\">[다음]</a>";
		pageHtml += "<a href=\"#\" onclick=\"goPage(" + ( currentPageNo == totalPageCount? totalPageCount : (currentPageNo+1) ) + "); return false;\">[다음]</a>";
